...
bool initPaint()
{
    if(isTable)
    {
        server.serveStatic("/paint.html", SPIFFS, "/paint.html"); //on ajoute le lien vers
    ...
    switch(type) {
        ...
        case WStype_TEXT:
            ...
            if (strcmp("paint", (const char *)str) == 0)  {
               // if(y==0 or y>=24)
                // PixelOn(x,y,CRGB(gamma8[g],gamma8[r],gamma8[b]));
                 //   else
                pixOnR(x,y,CRGB(gamma8[r],gamma8[g],gamma8[b]));
                //FastLED.show();
                // Serial.println("on va a droite");
  
            }
            if (strcmp("line", (const char *)str) == 0)  {
                 sscanf ((const char *)payload,"%s %d %d %d %d %d %d %d",str,&x,&y,&x1,&y1,&r,&b,&g);
                // if(y==0 or y>=24)
                // PixelOn(x,y,CRGB(gamma8[g],gamma8[r],gamma8[b]));
                //   else
                glbDrawLineR(x,y,x1,y1,CRGB(gamma8[r],gamma8[g],gamma8[b]));
                //FastLED.show();
                // Serial.println("on va a droite");
